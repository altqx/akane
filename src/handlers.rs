...
use tracing::error;
use uuid::Uuid;
use crate::types::{ AppState, UploadResponse, ProgressResponse, ProgressUpdate };
use crate::video::{ get_video_duration, get_video_height, get_variants_for_height, encode_to_hls };
...
pub async fn upload_video(
    ...
    let available_resolutions: Vec<String> = variants
        .iter()
        .map(|v| v.label.clone())
        .collect();
    ...
    encode_to_hls(&video_path, &hls_dir, &state.progress, &upload_id).await.map_err(|e|
        internal_err(e)
    )?;
